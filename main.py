...
def parse_args(args=None):
    parser = argparse.ArgumentParser(
        description='Training and Testing RULE_Net Models',
    ...
    parser.add_argument('--num_layers', type=int, default=3, help='length of rules')
    # parser.add_argument('--itertrain', type=bool, default= False, help='')
...
def save_model(model, optimizer, save_variable_list, args):
    ...
    entity_embedding = model.EmbeddingLayer.entity_embedding.detach().cpu().numpy()
    np.save(
        os.path.join(args.save_path, 'entity_embedding'),
        entity_embedding
    )

    relation_embedding = model.EmbeddingLayer.relation_embedding.detach().cpu().numpy()
    np.save(
...
def main(args):
    ...
    for name, param in rule_model.named_parameters():
    ...
    if args.do_train:
        ...
        optimizer = torch.optim.Adam(
            filter(lambda p: p.requires_grad, rule_model.parameters()),
    ...
    if args.recover_rule:
        ...
        for i, data in enumerate(train_dataloader_tail):
            ...
            tail = data[0].numpy()[0][1]
            print (i)
          
            weight2 = weight2.detach().cpu().numpy().squeeze() #(b, L)
            # weight2 = np.mean(weight2, 0)
            for l in range(1, len(r_atten)):    
                temp = list()
                for i in range(l+1):                
                    w = r_atten[i].detach().cpu().numpy().squeeze() # (r)
                    number = np.argsort(-w)
                    top = (number[0], w[number[0]])
                    # top3 = {number[k]:w[number[k]] for k in range(3)}
                    temp.append(top)           
                # print(temp)
                dnum = []
                rule_weight = 0
                for num, weight in temp:
                    dnum.append(num)
                    rule_weight = rule_weight + weight
                rule_weight  = rule_weight/len(temp)
                rule_weight  = weight2[l] * rule_weight
                if (tuple(dnum),tail) not in result:
                    result[(tuple(dnum),tail)] = [rule_weight]
                else:
                    result[(tuple(dnum),tail)].append(rule_weight)
    ...
    if args.init_checkpoint:
        ...
        if args.do_train:
            ...
            # change_steps = checkpoint['change_steps']
            optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    else:
        logging.info('Ramdomly Initializing Rulenet Model...')
        init_step = 0
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            if step == change_steps:
                logging.info('Change train  model at step %d' % ( step))
            ...
            if step >= change_steps:
                ...
                log = rule_model.train_step(rule_model, optimizer, train_iterator, args, True)
            else:
                log = rule_model.train_step(rule_model, optimizer, train_iterator, args, False)
            ...
            if step % args.save_checkpoint_steps == 0:
                ...
                save_model(rule_model, optimizer, save_variable_list, args)
